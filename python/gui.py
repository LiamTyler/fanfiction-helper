...
from story import EncodeNum, EncodeStr, StoryFromNetworkBytes
...
class MainWindow( QMainWindow ):
    ...
    def OpenStoryUrl( self, storyListItem ):
        url = storyListItem.story.GetUrl()
    ...
    def FandomHome( self, fandom ):
        ...
        i = 0
        while i < len( split ):
            story, i = StoryFromNetworkBytes( split, i )
            stories.append( story )
        
        numStories = len( stories )
        vlist = QListWidget( self )
        vlist.setWordWrap( True )
        for i in range( numStories ):
            vlist.addItem( StoryListItem( stories[i] ) )
